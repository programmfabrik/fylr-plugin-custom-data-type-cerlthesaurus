ZIP_NAME ?= "customDataTypeCERLThesaurus.zip"
PLUGIN_NAME = "custom-data-type-cerlthesaurus"

# coffescript-files to compile
COFFEE_FILES = CustomDataTypeCERLThesaurus.coffee \
	CustomDataTypeCERLThesaurusFacet.coffee \
	CERLThesaurusUtil.coffee
...
build: clean buildinfojson ## clean, compile, copy files to build folder
			...
			cat src/tmp/CustomDataTypeCERLThesaurus.js >> build/$(PLUGIN_NAME)/webfrontend/customDataTypeCERLThesaurus.js
			cat src/tmp/CustomDataTypeCERLThesaurusFacet.js >> build/$(PLUGIN_NAME)/webfrontend/customDataTypeCERLThesaurus.js
			cat src/tmp/CERLThesaurusUtil.js >> build/$(PLUGIN_NAME)/webfrontend/customDataTypeCERLThesaurus.js

			cp src/updater/CERLThesaurusUpdater.js build/$(PLUGIN_NAME)/updater/CERLThesaurusUpdater.js # build updater
			cat src/tmp/CERLThesaurusUtil.js >> build/$(PLUGIN_NAME)/updater/CERLThesaurusUpdater.js
			cp package.json build/$(PLUGIN_NAME)/package.json
			cp -r node_modules build/$(PLUGIN_NAME)/
			rm -rf src/tmp # clean tmp

			cp l10n/customDataTypeCERLThesaurus.csv build/$(PLUGIN_NAME)/l10n/customDataTypeCERLThesaurus.csv # copy l10n			
			
			cp src/webfrontend/css/main.css build/$(PLUGIN_NAME)/webfrontend/customDataTypeCERLThesaurus.css # copy css
