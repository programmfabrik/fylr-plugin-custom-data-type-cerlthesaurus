...
function hasChanges(objectOne, objectTwo) {
  ...
  for (let i = 0; i < ref.length; i++) {
    let key = ref[i];
    if (!CERLThesaurusUtil.isEqual(objectOne[key], objectTwo[key])) {
      return true;
    }
  }
  return false;
    ...
    fetch(url, {
      ...
      .then(response => {
        if (response.ok) {
          ...
          console.error("CERLThesaurus-Updater: Fehler bei der Anfrage an /config ");
        }
      })
      .catch(error => {
        console.error(error);
...
function getNewCustomExpiresAt() {
  ...
  const customExpirationConfig = info?.config?.plugin?.['custom-data-type-cerlthesaurus']?.config?.update_cerlthesaurus?.custom_expires_days || 1
...
main = (payload) => {
  ...
  switch (payload.action) {
    ...
    case "update":
      ////////////////////////////////////////////////////////////////////////////
      // run CERLThesaurus-api-call for every given uri
      ////////////////////////////////////////////////////////////////////////////
      ...
      URIList.forEach((uri) => {
        ...
        let cerlthesaurusID = decodeURIComponent(uri)
        cerlthesaurusID = cerlthesaurusID.split("/")
        cerlthesaurusID = cerlthesaurusID.pop()
        const dataRequestUrl = `https://ws.gbv.de/suggest/cerl_thesaurus/?searchstring=${cerlthesaurusID}&type=placename&count=1`;
        const options = {
          headers: {
            Accept: 'application/json',
            'User-Agent': 'Node'
      ...
      Promise.all(requestUrls).then(function (responses) {
        ...
        payload.objects.forEach((result, index) => {
          ...
          let newCdata = structuredClone(payload.objects[index].data);
          ...
          if (matchingRecordData) {
            console.error("CERLThesaurusUpdater: if matchingRecordData")
            ///////////////////////////////////////////////////////
            // conceptName, conceptURI, conceptSource, _standard, _fulltext, facet
            resultJSON = matchingRecordData.data;
            resultObject = Array.isArray(resultJSON) ? resultJSON[0] : null
            if (resultObject) {
              console.error("CERLThesaurusUpdater: if resultObject")
              // save conceptName
              // newCdata.conceptName = '??_' + resultObject.aLabel;
              newCdata.conceptName = CERLThesaurusUtil.getConceptNameFromObject(resultObject);
              // save conceptURI
              newCdata.conceptURI = CERLThesaurusUtil.getConceptURIFromObject(resultObject);
              // save _fulltext
              newCdata._fulltext = CERLThesaurusUtil.getFullTextFromObject(resultObject, databaseLanguages);
              // save _standard
              newCdata._standard = CERLThesaurusUtil.getStandardTextFromObject(null, resultObject, newCdata, databaseLanguages);
...
(() => {
  ...
  if (info?.config?.plugin?.['custom-data-type-cerlthesaurus']?.config?.update_cerlthesaurus?.restrict_time === true) {
    plugin_config = info.config.plugin['custom-data-type-cerlthesaurus'].config.update_cerlthesaurus;
  ...
  if (access_token) {
    ...
    getConfigFromAPI().then(config => {
      ...
      ////////////////////////////////////////////////////////////////////////////
      // availabilityCheck for api
      ////////////////////////////////////////////////////////////////////////////

      const testURL = "https://ws.gbv.de/suggest/cerl_thesaurus/?searchstring=cnp01900169&type=placename&count=1"
      const options = {
        headers: {
          Accept: 'application/json',
          'User-Agent': 'Node'
      ...
      https.get(testURL, options, res => {
        ...
        res.on('end', () => {
          ...
          if (!Array.isArray(testJSON)) {
            return console.error("Error: Json parse complete, result is an array.")
          }

          if (testJSON.length < 1) {
            return console.error('Error: Result array is empty.')
          }
          if (!testJSON[0].cerlID === 'cnp01900169') {
            return console.error('Error: First element of result array does not contain requested id.')
          }
        ...
        console.error('Error while receiving data from CERL-Thesaurus-API: ', err.message);
      });
    }).catch(error => {
